...
<body>
    ...
    <a href="inscription.php" class="button lien btn btn-info">créer un compte employés</a>
        ...
        <nav class="nav bg-light justify-content-center">
            ...
            <a href="admin_membre.php" class="nav-link text-dark">liste des employés</a>
        </nav>

        <!--home-->
        <div class="container p-4">
          <article class="col-md-12">
         <?php // Récupérer le contenu du champ "service" de la table "users"
$sql = "SELECT text FROM service";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Récupérer le nouveau contenu du champ "service"
    $newService = $_POST["service"];

    // Mettre à jour le champ "service" dans la table "users"
    $sqlUpdate = "UPDATE service SET text = :newService";
...
$conn = null;
?>
    <!-- Afficher le contenu du champ "service" -->
    <h2>Contenu du service :</h2>
    <?php echo nl2br($service); ?>

    <!-- Formulaire pour la modification du champ "service" -->
    <h2>Modifier le contenu du service :</h2>
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <textarea name="service" rows="10" cols="75"><?php echo $service; ?></textarea>
        <br>
